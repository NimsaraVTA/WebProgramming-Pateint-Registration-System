...
if(isset($_GET['search'])){
    ...
    $query="SELECT * FROM ward_contact WHERE w_name LIKE '%{$search}%'";

}else if(empty($_GET['search'])){

$query="SELECT * FROM ward_contact where is_deleted=0 ORDER BY w_name";
...
if($users){
    ...
    while($user=mysqli_fetch_assoc($users)){
      $userlist.="<tr>";
        $userlist.="<td>{$user['w_name']}</td>";
        $userlist.="<td>{$user['w_no']}</td>";
        $userlist.="<td>{$user['w_details']}</td>";
        $userlist.="<td><a href=\"modify_w_contact.php?w_name={$user['w_name']}\">Edit</a></td>";
        $userlist.="<td><a href=\"delete_w_contact.php?w_name={$user['w_name']}\">Delete</a></td>";
      $userlist.='<tr>';    
...
<div class="topnav" id="myTopnav">
  ...
  <a href="doctor_contacts.php">Add Ward Contacts</a>
...
<h2 style="text-align:center; padding:15px;">Wards' Contact List</h2>
